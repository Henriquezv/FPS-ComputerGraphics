...
bool bulletCollidedEnemy(glm::vec4 bullet, TEnemy enemy){
    ...
    if ((enemy.rotation.y >= PI/4 && enemy.rotation.y <= 3*PI/4)
        || (enemy.rotation.y >= 3*-PI/4 && enemy.rotation.y <= -PI/4))
        {
        collided = (bullet.x > (enemy_obj.bbox_min.z*enemy.scale.x) + enemy.position.x
                    && bullet.x < (enemy_obj.bbox_max.z*enemy.scale.x) + enemy.position.x)
            && (bullet.y > (enemy_obj.bbox_min.y*enemy.scale.y) + enemy.position.y
                && bullet.y < (enemy_obj.bbox_max.y*enemy.scale.y) + enemy.position.y)
            && (bullet.z > (enemy_obj.bbox_min.x*enemy.scale.z) + enemy.position.z
                && bullet.z < (enemy_obj.bbox_max.x*enemy.scale.z) + enemy.position.z);
...
bool bulletCollidedBunny(glm::vec4 bullet, TBunny bunny){
    ...
    bool EnemyCollidedBunny(TEnemy enemy, TBunny bunny){
        ...
        if ((bunny.rotation.y >= PI/4 && bunny.rotation.y <= 3*PI/4)
            || (bunny.rotation.y >= 3*-PI/4 && bunny.rotation.y <= -PI/4)) {
            ...
            collided = ((bunny_obj.bbox_min.x*bunny.scale.x) + bunny.position.x
                        <= (enemy_obj.bbox_max.x*enemy.scale.x) + enemy.position.x
                 && (bunny_obj.bbox_max.x*bunny.scale.x) + bunny.position.x
                        >= (enemy_obj.bbox_min.x*enemy.scale.x) + enemy.position.x)
                 && ((bunny_obj.bbox_min.y*bunny.scale.y) + bunny.position.y
                     <= (enemy_obj.bbox_max.y*enemy.scale.y) + enemy.position.y
                 && (bunny_obj.bbox_max.y*bunny.scale.y) + bunny.position.y
                     >= (enemy_obj.bbox_min.y*enemy.scale.y) + enemy.position.y)
                 && ((bunny_obj.bbox_min.z*bunny.scale.z) + bunny.position.z
                     <= (enemy_obj.bbox_max.z*enemy.scale.z) + enemy.position.z
                 && (bunny_obj.bbox_max.z*bunny.scale.z) + bunny.position.z
                     >= (enemy_obj.bbox_min.z*enemy.scale.z) + enemy.position.z);
        }
        return collided;
    }
...
int main(int argc, char* argv[])
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::vec4 u = crossproduct(camera_up_vector, w)
                    /norm(crossproduct(camera_up_vector, w));
